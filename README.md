...
---
# 실습
1. 도커 이미지는 컨테이너를 만들기 위해 필요한 설정, 종속성을 가진 소프트웨어 패키지
2. Dockerhub에 다른 사람들이 만든 도커이미지를 활용할 수 있고, 새 이미지를 만들 수 있음 (`docker create [이미지 이름]`)
...
#### 도커 파일 생성 순서
1. 베이스 이미지 명시 (파일 스냅샷)
- 베이스 이미지 : 여러 레이어로 구성된 도커 이미지의 기반이 되는 부분. 레이어는 중간 단계의 이미지
- 레이어를 추가 시 레이어 캐싱
- 바탕화면에 신규 폴더 생성 후 ide에서 오픈
- 파일 이름은 `Dockerfile` 로 해야 오류 없음
```shell
#베이스 이미지
# 베이스 이미지 중 기초
# <이미지 이름> : <태그> 형식으로 작성, 태그 없을 시 최신버전 사용
FROM baseImage
...
CMD ["executeable"]
```
2. 추가적으로 필요한 파일을 다운받는 명령어 작성 (파일 스냅샷)
3. 컨테이너 시작 시 실행 될 명령어 명시 (실행 시 명령어)

### 2. 도커 클라이언트
- 도커 파일에 입력된 내용을 도커 클라이언트로 전달 (도커 서버에서 인식 위함)
- `docker build ./` 또는 `docker build .` -> 도커 버전이 변경되어 `DOCKER_BUILDKIT=0 docker build .` 사용
- Build 명령어는 해당 디렉토리 내에서 dockerfile 파일을 도커 클라이언트로 전달
- 이미지 별명 : `-t` 옵션 사용, 규칙 : `-t 도커아이디/저장소:버전`
### 3. 도커 서버
- 도커 클라이언트에 전달된 모든 중요한 역할을 하는 곳
### 4. 이미지 생성